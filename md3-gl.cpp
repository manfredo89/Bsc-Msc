...
vector<double> r, v, a;      // positions, velocities, accelerations
...
vector<double> drPair;       // vector separations of each pair (i,j)
vector<double> rSqdPair;     // squared separation of each pair (i,j)
int updateInterval = 10;  // number of time steps between updates of pair list

// declare functions repeated from md-3D-2.cpp
void initialize();
...
void initialize() {
    ...
    drPair = vector<double>(3*nPairs);
    rSqdPair = vector<double>(nPairs);
}

void computeSeparation (int i, int j, vector<double>& dr, double& rSqd) {
...
void velocityVerlet(double dt) {
    ...
    // update positions, assuming accelerations have been computed
    for (int i = 0; i < 3*N; i++)
